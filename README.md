...
為了避免資料庫無限增長（Supabase 免費方案限制 500 MB），系統會自動清理舊資料。
...
- **小型使用**（節省空間）：`max_articles: 50, max_notifications: 200`
- **中型使用**（預設）：`max_articles: 100, max_notifications: 500`
- **大型使用**（長期保存）：`max_articles: 200, max_notifications: 1000`

### 停用自動清理

如果不想自動清理，將 `enabled` 設為 `false`：
...
更多關於清理功能的詳細說明，請參考 [DATABASE_CLEANUP.md](DATABASE_CLEANUP.md)
